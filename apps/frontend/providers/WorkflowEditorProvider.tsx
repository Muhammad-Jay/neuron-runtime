...
export const WorkflowEditorContext =
  createContext<WorkflowEditorContextType | null>(null);
...
export function WorkflowEditorProvider({
  ...
  const [isGlobalVariableSheetOpen, setIsGlobalVariableSheetOpen] =
    useState(false);
  const [isEditorPanelOpen, setIsEditorPanelOpen] = useState(false);
  const [isDeployWorkflowDialogOpen, setIsDeployWorkflowDialogOpen] =
    useState(false);
  ...
  const [editorState, workflowEditorDispatch] = useReducer(
          ...
          graph: {
            ...
            nodes: {
              ...
              [action.id]: {
                ...
                config: {
                  ...state.graph.nodes[action.id].config,
                  ...action.payload,
                },
              },
            },
  ...
  const loadDeployment = async () => {
    try {
      const token = await getSession();
      const deployment: any[] = await getDeployWorkflowRequest(
        workflowId,
        token
      );
      if (deployment) {
  ...
  const deployWorkflow = async (data: {
    ...
    try {
      setIsDeploying(true);
      const token = await getSession();
      const deployed = await deployWorkflowRequest(
        workflowId,
        {
          ...data,
